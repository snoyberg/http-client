...
ungzip inner = do
    zstr <- liftIO c_create_z_stream -- FIXME finalizer
    ungzip' zstr inner

ungzip' :: MonadIO m => ZStream -> Enumeratee S.ByteString S.ByteString m b
ungzip' zstr (Continue k) = do
    x <- head
    case x of
        Nothing -> do
            
            liftIO $ c_free_z_stream zstr
            return $ Continue k
        Just bs -> do
            chunks <- liftIO $ unsafeUseAsCStringLen bs $ \(cstr, len) -> do
                putStrLn $ "zlib: Got a bs length " ++ show (S.length bs)
